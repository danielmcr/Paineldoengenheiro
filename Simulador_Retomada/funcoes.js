...
function calculaPercentualGlobal() {
  ...
  // Calculando os incrementos percentuais
  let somaValores = ValorHab + ValorEqui + ValorInfr;

  let IncHab = ValorHab / somaValores;
  let IncEqui = ValorEqui / somaValores;
  let IncInfr = ValorInfr / somaValores;

  // Atualizando o DOM com os novos valores percentuais
  let Hab = document.getElementById("PercHab").value;
  Hab = parseNumberFromString(Hab);
  let Equi = document.getElementById("PercEqui").value;
  Equi = parseNumberFromString(Equi);
  let Infr = document.getElementById("PercInfr").value;
  Infr = parseNumberFromString(Infr);

  let PercTotal = document.getElementById("PercTotal");

  let resultadoFinal = IncHab * Hab + IncEqui * Equi + IncInfr * Infr;

  PercTotal.textContent = resultadoFinal.toFixed(2) + "% (Global)";
}
...
async function calcularInvolucao() {
  try {
    ...
    let [totalMeses, totalMesesAtual] = await calcularMesesEntreDatas(
      dataInicial,
      dataFinal,
      dataSimulacao
    );
    ...
    let involucaoCalculada = await formulaInvolucao(
      totalMeses,
      totalMesesAtual
    );

    let RaeInvolucao =
      document.getElementById("cboSN").options[
        document.getElementById("cboSN").selectedIndex
      ].value;

    if (RaeInvolucao === "SIM") {
      involucaoCalculada = 0;
      document.getElementById("Involucao").textContent =
        involucaoCalculada.toFixed(2);
...
async function atualizaValores() {
  try {
    ...
    const [ValorSPG, ValorFC, ValorEB, ValorIE, ValorCpl, somaPercSNI] =
      await atualizaValorSNI();
    ...
    if (varSinapi !== null) {
      ...
      document.getElementById("PerInfrNITab1").textContent =
        aeInfrNI.toFixed(2);

      let ValorFinalHab1 = (varSinapi / 100 + 1) * (aeHab / 100) * ValorHab;
      //console.log(ValorFinalHab);
      document.getElementById("ValorCalcHab1").textContent =
        formatarComoMoeda(ValorFinalHab1);
      let ValorFinalEqui1 = (varSinapi / 100 + 1) * (aeEqui / 100) * ValorEqui;
      document.getElementById("ValorCalcEqui1").textContent =
        formatarComoMoeda(ValorFinalEqui1);
      let ValorFinalInfr1 = (varSinapi / 100 + 1) * (aeInfr / 100) * ValorInfr;
      document.getElementById("ValorCalcInfr1").textContent =
        formatarComoMoeda(ValorFinalInfr1);
      let ValorFinalSN1 = (aeSN / 100) * ValorSN;
      document.getElementById("ValorCalcSN1").textContent =
        formatarComoMoeda(ValorFinalSN1);
      let ValorFinalInfrNI1 =
        (varSinapi / 100 + 1) * (aeInfrNI / 100) * ValorInfrNI;
      document.getElementById("ValorCalcInfrNI1").textContent =
        formatarComoMoeda(ValorFinalInfrNI1);

      let ValorFinalTotal1 =
        ValorFinalHab1 +
        ValorFinalEqui1 +
        ValorFinalInfr1 +
        ValorFinalSN1 +
      ...
      document.getElementById("ValorCalcTotal1").textContent =
        formatarComoMoeda(ValorFinalTotal1);
      document.getElementById("%HabFinal1").textContent = (
        (ValorFinalHab1 / ValorFinalTotal1) *
        100
      ).toFixed(2);
      document.getElementById("%EquiFinal1").textContent = (
        (ValorFinalEqui1 / ValorFinalTotal1) *
        100
      ).toFixed(2);
      document.getElementById("%InfrFinal1").textContent = (
        (ValorFinalInfr1 / ValorFinalTotal1) *
        100
      ).toFixed(2);
      document.getElementById("%InfrNIFinal1").textContent = (
        (ValorFinalInfrNI1 / ValorFinalTotal1) *
        100
      ).toFixed(2);
      document.getElementById("%SNFinal1").textContent = (
        (ValorFinalSN1 / ValorFinalTotal1) *
        100
      ).toFixed(2);

      let ValorBDI1 = ValorFinalTotal1 * (BDI / 100);
      document.getElementById("ValorBDI1").textContent =
        formatarComoMoeda(ValorBDI1);

      let ValorFinalTotalBDI1 = ValorFinalTotal1 * (1 + BDI / 100);
      document.getElementById("ValorCalcTotalBDI1").textContent =
        formatarComoMoeda(ValorFinalTotalBDI1);
      document.getElementById("ValorCalcTotalResumo1").textContent =
        formatarComoMoeda(ValorFinalTotalBDI1);
      ...
      document.getElementById("PerInfrNITab2").textContent =
        aeInfrNI.toFixed(2);
      ...
      let ValorFinalHab2 =
        (aeHab / 100) *
        (parseFloat(valorFinal) * AEq * (100 / (100 - somaPercSNI)));
      //console.log(ValorFinalHab2);
      document.getElementById("ValorCalcHab2").textContent =
        formatarComoMoeda(ValorFinalHab2);
      let ValorFinalEqui2 =
        ((aeHab / 100) *
          (parseFloat(valorFinal) * AEq * (100 / (100 - somaPercSNI))) *
          ValorEqui) /
        ValorHab;
      document.getElementById("ValorCalcEqui2").textContent =
        formatarComoMoeda(ValorFinalEqui2);
      let ValorFinalInfr2 =
        ((aeHab / 100) *
          (parseFloat(valorFinal) * AEq * (100 / (100 - somaPercSNI))) *
          ValorInfr) /
        ValorHab;
      document.getElementById("ValorCalcInfr2").textContent =
        formatarComoMoeda(ValorFinalInfr2);
      let ValorFinalSN2 = (aeSN / 100) * ValorSN;
      document.getElementById("ValorCalcSN2").textContent =
        formatarComoMoeda(ValorFinalSN2);
      let ValorFinalInfrNI2 =
        (varSinapi / 100 + 1) * (aeInfrNI / 100) * ValorInfrNI;
      document.getElementById("ValorCalcInfrNI2").textContent =
        formatarComoMoeda(ValorFinalInfrNI2);
      let ValorFinalTotal2 =
        ValorFinalHab2 +
        ValorFinalEqui2 +
        ValorFinalInfr2 +
        ValorFinalSN2 +
      ...
      document.getElementById("ValorCalcTotal2").textContent =
        formatarComoMoeda(ValorFinalTotal2);
      document.getElementById("%HabFinal2").textContent = (
        (ValorFinalHab2 / ValorFinalTotal2) *
        100
      ).toFixed(2);
      document.getElementById("%EquiFinal2").textContent = (
        (ValorFinalEqui2 / ValorFinalTotal2) *
        100
      ).toFixed(2);
      document.getElementById("%InfrFinal2").textContent = (
        (ValorFinalInfr2 / ValorFinalTotal2) *
        100
      ).toFixed(2);
      document.getElementById("%InfrNIFinal2").textContent = (
        (ValorFinalInfrNI2 / ValorFinalTotal2) *
        100
      ).toFixed(2);
      document.getElementById("%SNFinal2").textContent = (
        (ValorFinalSN2 / ValorFinalTotal2) *
        100
      ).toFixed(2);
      document.getElementById("ValorBDI2").textContent = formatarComoMoeda(
        ValorFinalTotal2 * (BDI / 100)
      );

      let ValorFinalTotalBDI2 = ValorFinalTotal2 * (1 + BDI / 100);
      document.getElementById("ValorCalcTotalBDI2").textContent =
        formatarComoMoeda(ValorFinalTotalBDI2);
      document.getElementById("ValorCalcTotalResumo2").textContent =
        formatarComoMoeda(ValorFinalTotalBDI2);
...
async function buscaSinapi() {
  try {
    const apiUrl =
      "https://servicodados.ibge.gov.br/api/v3/agregados/{agregado}/periodos/{periodos}/variaveis/51?localidades=N3[{UF}]&classificacao=314[{Tipo}]|41[{Padrao}]";
    ...
    valorInicial = JSON.stringify(
      data[0].resultados[0].series[0].serie[anomesinicial]
    )
      .replace('"', "")
      .replace('"', "");
    ...
    valorFinal = JSON.stringify(
      data[0].resultados[0].series[0].serie[anomesfinal]
    )
      .replace('"', "")
      .replace('"', "");
...
function calcularInvolucaoGrafico(totalMeses, totalMesesAtual) {
  var resultado = Math.exp(
    0.0245567 * totalMeses + 0.0338926 * totalMesesAtual
  );
...
async function criarGraficoAmCharts() {
  try {
    ...
    for (var i = 0; i <= 30; i++) {
      ...
      data.push({
        date: novoMes,
        value1: valorInicial1,
        value2: valorInicial2, //+ valorInvolucao
      });
    }
    chart.data = data;
    ...
    // Após definir os dados do gráfico e os eixos, crie as duas séries de linhas:

    // Cria a primeira série de linhas para valorInicial1
    var series1 = chart.series.push(new am4charts.LineSeries());
    series1.dataFields.valueY = "value1";
    series1.dataFields.dateX = "date";
    series1.stroke = am4core.color("#ff8726"); // Cor da linha para valorInicial1
    series1.strokeWidth = 3;
    series1.tooltipText = "Metodologia 1: {value1}";
    series1.name = "Metodologia 1"; // Nome opcional para a legenda

    // Cria a segunda série de linhas para valorInicial2
    var series2 = chart.series.push(new am4charts.LineSeries());
    series2.dataFields.valueY = "value2";
    series2.dataFields.dateX = "date";
    series2.stroke = am4core.color("#00D8FF"); // Cor da linha para valorInicial2
    series2.strokeWidth = 3;
    series2.tooltipText = "Metodologia 2: {value2}";
    series2.name = "Metodologia 2"; // Nome opcional para a legenda

    // Habilita a legenda (opcional)
    chart.legend = new am4charts.Legend();
    chart.legend.labels.template.fill = am4core.color("#5C5C5C");
...
function atualizaTextoData() {
  ...
  const mesEscolhidoTexto =
    cboMesFinal.options[cboMesFinal.selectedIndex].textContent;
  const anoEscolhidoValor =
    cboAnoFinal.options[cboAnoFinal.selectedIndex].value;

  // Atualize o conteúdo de texto do elemento que alerta sobre a data
  document.getElementById("AlertaData1").textContent =
    "Atenção! Valor ESTIMADO apenas para " +
    mesEscolhidoTexto +
    " de " +
    anoEscolhidoValor +
    ".";
  document.getElementById("AlertaData2").textContent =
    "Atenção! Valor ESTIMADO apenas para " +
    mesEscolhidoTexto +
    " de " +
    anoEscolhidoValor +
...
document.addEventListener("DOMContentLoaded", function () {
  var toggleButton1 = document.querySelector(".toggle-button1");
  var detalhes1 = document.getElementById("detalhesValoresCalculados1");

  toggleButton1.addEventListener("click", function () {
    var isHidden = detalhes1.style.display === "none";
    detalhes1.style.display = isHidden ? "block" : "none";
    toggleButton1.textContent = isHidden
      ? "Ocultar Detalhes"
      : "Mostrar Detalhes";
  });
});
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  toggleButton2.addEventListener("click", function () {
    ...
    toggleButton2.textContent = isHidden2
      ? "Ocultar Detalhes"
      : "Mostrar Detalhes";
  });
});
